      ...
      <div <?php echo $this->itemscope.' '.$this->itemtype_RealEstateAgent; ?> class="wpl_profile_container <?php echo (isset($this->property_css_class) ? $this->property_css_class : ''); ?>" id="wpl_profile_container<?php echo $profile['data']['id']; ?>">
			...
			<div class="wpl_profile_picture"
				<?php
					...
					{
						echo '<a href="'.$profile["profile_link"].'"><img '.$this->itemprop_image.'. src="'.$profile["profile_picture"]["url"].'" alt="'.$agent_name.' '.$agent_l_name.'" /></a>';
					}
					else
					{
			...
			<div class="wpl_profile_container_title">
  			...
  			<ul>
				...
				if (isset($profile['main_email_url']) and wpl_global::get_setting('profile_email_type') == '0') echo '<li class="email"><img '.(isset($this->itemprop_email) ? $this->itemprop_email : "").'. src="' . $profile["main_email_url"] . '" alt="' . $agent_name . ' ' . $agent_l_name . '" /></li>';
